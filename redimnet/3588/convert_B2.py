...
def random_sample_path() -> str:
    arr = np.random.randn(1, 1, 72, FRAMES).astype(np.float32)
...
def load_logmel(path: str) -> np.ndarray:
    ...
    if x.shape[1] != 1:                   # assume NHWC → NCHW
        if x.shape[-1] != 1 or x.shape[1] != 72:
            raise ValueError(f'Cannot interpret log-Mel shape {x.shape}')
        x = x.transpose(0, 3, 1, 2)

    if x.shape[2] != 72 or x.shape[3] != FRAMES:
        raise ValueError(f'Expected [1,1,72,{FRAMES}], got {x.shape}')
    return x
...
def main():
    ...
    if rknn.load_onnx(onnx_path,
                      inputs=['log_mel'],
                      input_size_list=[[1, 1, 72, FRAMES]]) != 0:
